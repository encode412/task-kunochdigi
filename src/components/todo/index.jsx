...
import { ChangeTodo, TodoForm, TodoItem } from "./_components";
...
const Todo = () => {
  ...
  return (
    <div className="flex flex-col pt-10 md:pt-20 pb-8 mx-10 md:my-20 gap-12 items-center bg-white px-[1.3rem] md:px-20 w-5/6 md:w-1/2 overflow-hidden my-auto md:mx-auto rounded-2xl">
      ...
      <div className="overflow-y-auto h-[300px] lg:h-[150px] py-6 px-4 space-y-6">

      {todos?.map((todo) =>
        todo.isEditing ? (
          <ChangeTodo editTodo={editTask} task={todo} />
        ) : (
          <TodoItem
            key={todo.id}
            task={todo}
